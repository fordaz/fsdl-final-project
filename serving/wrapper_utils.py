...
def append_valid_annotations(sampled_annotations, valid_annotations):
    ...
    for sampled_annotation in sampled_annotations:
        try:
            ...
            has_key_properties = all(
                map(
                    lambda property: property in sampled_annotation,
                    ["box", "text", "id", "label"],
                )
            )
...
def generate_syn_page(
    ...
    while len(valid_annotations) < num_annot and attempts < max_attemps:
        sampled_token_idxs = sample_from_gru_model(
            model, vectorizer, device, batch_size, max_annot_length, temperature
        )
